...
- [Medusa backend v2](https://docs.medusajs.com/learn/installation)
...
3\. In `medusa-config.ts` (or `.js`) add the fulfillment module provider:
        ...
        providers: [
          {
            ...
            options: {
              ...
              email: process.env.SHIPROCKET_EMAIL, //(required if token not provided)
              password: process.env.SHIPROCKET_PASSWORD, //(required if token not provided)
              token: "", //(optional. use if you manage tokens yourself)
...
2\. Enable the fulfillment provider in the Admin. You can refer to [this User Guide](https://docs.medusajs.com/user-guide/regions/providers) to learn how to do that. Alternatively, you can use the Admin APIs.
...
If your business logic requires a different functionality than described as above, you need to modify `service.ts` in `src/providers/shiprocket` and use the utility functions created in `src/utils`.
...
1\. Not yet re-validated on Medusa v2. Please report any return/fulfillment event gaps.

2\. When a return is requested through Storefront Endpoint, it throws an error in the server saying "Cannot read properties of undefined (reading 'items')" even if item array is defined as per the endpoint's requirements. (Observed on older Medusa versions; needs re-validation on v2.)
...
- Fulfillment Module Provider: https://docs.medusajs.com/resources/commerce-modules/fulfillment
- Module Provider options: https://docs.medusajs.com/resources/commerce-modules/fulfillment/module-options
- Example: Manual https://docs.medusajs.com/resources/commerce-modules/fulfillment/manual
