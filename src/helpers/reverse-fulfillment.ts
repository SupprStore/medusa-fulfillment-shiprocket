...
async function reverseFulfillment(
  reverseData: ReverseFulfillmentData
): Promise<any> {
  const {
    options,
    client,
  ...
  const returnShipment: any = {
    courier_id: courier_id,
    order_id: `${orderDisplayId + 'R'}`,
    ...
    order_items: await Promise.all(
      (reverseData.returnItems?.length
        ? reverseData.returnItems
        : fromOrder.items
      ).map(async (item) => {
    ...
    total_discount: normalizeAmount(orderDiscountTotal, fromOrder.currency_code),
